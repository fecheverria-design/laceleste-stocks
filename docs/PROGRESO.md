...
- ⏳ **Inventario inicial** (lo único que queda): tras el fix, FABRICA quedó 393 neg / 24 pos / 7 cero — el historial de 3c no arranca desde stock cero, así que el neto da corrido a negativo. Falta cargar el **inventario actual** como punto de partida (una recepción/ajuste por producto con la cantidad contada hoy). Definir con J cómo viene ese dato (¿Excel de inventario?).
