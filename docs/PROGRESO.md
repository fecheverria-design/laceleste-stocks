...
- **Pendiente**: `import:ubicaciones` (depósitos/áreas de 3c) y `import:movimientos`. Necesito el formato del Excel de movimientos + la lista de ubicaciones de 3c. Orden: catálogos+ubicaciones → movimientos (referencian productos por codigo_3c y ubicaciones por dep_id_3c). Históricos entran CONFIRMADO con `nro_3c` para idempotencia. **Precios = fase futura** (columna nueva en productos).
